...
const sections = [
  {
    ...
    items: [
      { icon: "⬡", label: "Rotate", desc: "Click and drag to spin the globe in any direction." },
      { icon: "⊕", label: "Zoom", desc: "Scroll up or down to zoom in and out." },
    ],
  },
  {
    heading: "Exploring Resorts",
    items: [
      { icon: "◉", label: "Resort pin", desc: "Click a pin to see a quick summary of that resort, including vertical drop, runs, and lifts." },
      { icon: "⬤", label: "Cluster", desc: "Click a numbered cluster to expand it and browse the resorts inside. Select any resort from the list for details." },
      { icon: "⊞", label: "Resort map", desc: "From a resort summary, click \"View Map\" to open a detailed 3D terrain map with slope boundaries." },
    ],
  },
  {
    heading: "Filtering",
    items: [
      { icon: "▤", label: "Pass filter", desc: "Toggle Ikon Pass or Epic Pass in the top-right panel to show only those resorts on the globe." },
      { icon: "◈", label: "Pass type", desc: "Filter by Full Pass or Limited access using the Pass Type toggles." },
      { icon: "▲", label: "Stats filters", desc: "Use the Min Vertical, Min Runs, and Min Lifts sliders to surface resorts that meet a minimum threshold." },
    ],
  },
];
    ...
    <>
      ...
      >
        <div
          ...
          style={{ background: "rgba(6, 20, 40, 0.97)", backdropFilter: "blur(16px)" }}
        >
          {/* Header */}
          ...
          <div className="px-6 py-5 space-y-6">
            {sections.map((section) => (
              <div key={section.heading}>
                <p className="text-white/35 text-[10px] uppercase tracking-widest mb-3">{section.heading}</p>
                <ul className="space-y-3">
                  {section.items.map((item) => (
                    <li key={item.label} className="flex items-start gap-3">
                      <span className="shrink-0 w-7 h-7 rounded-lg flex items-center justify-center text-white/60 text-sm border border-white/10" style={{ background: "rgba(255,255,255,0.05)" }}>
                        {item.icon}
                      </span>
                      <div>
